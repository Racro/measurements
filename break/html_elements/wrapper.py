...
extn_lst = ['adblock', 'ublock', 'privacy-badger',
       ...
       "user-agent"]
# extn_lst = ['']
SIZE = 12 # number of browser windows that will open
...
def run(sites, extn, return_dict, l):
    ...
    try:
        cmd = ["python3", "crawler.py", "--extension", extn, str(sites)]
    ...
    # l.release()
    if stdout:
        # print(f'wrapper1 ---- {stdout}')
        return_dict[extn].extend(ast.literal_eval(stdout))
    l.release()
...
if __name__ == "__main__":
    try:
        ...
        for key in updated_dict.keys():
            latest_list.append(updated_dict[key][0])
        # latest_list = latest_list[:50]
        # latest_list = ['http://www.google.com']
        chunks_list = list(divide_chunks(latest_list, math.ceil(len(latest_list)/SIZE)))
        ...
        for extn in extn_lst:       
            ...
            for i in range(len(chunks_list)):
                p = multiprocessing.Process(target=run, args=(chunks_list[i], extn, return_dict, multiprocessing.Lock(),))
                jobs.append(p)
