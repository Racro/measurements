    ...
    def initialize(self, options, num_tries, url):
        ...
        while num_tries > 0:
            try:
                ...
                # service = Service(executable_path='/usr/bin/chromedriver', service_args=["--verbose", f"--log-path={log_file_path}"])
                version = read_version_from_cmd("./chrome_120/chrome --version", PATTERN["chromium"])
                service = Service(ChromeDriverManager(driver_version=version).install(), service_args=["--verbose", f"--log-path={log_file_path}"])
